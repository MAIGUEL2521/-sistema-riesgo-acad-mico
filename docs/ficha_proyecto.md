...
- **Justificación de la métrica:** El dataset presenta un desbalance leve entre las categorías de rendimiento. El F1-score macro calcula el F1 de cada clase de forma independiente y promedia los resultados, garantizando que ninguna categoría sea ignorada durante la evaluación. Esto es preferible al accuracy cuando las clases no están perfectamente balanceadas, ya que el accuracy puede ser engañosamente alto si el modelo predice siempre la clase mayoritaria.
...
- **Descripción general:** Dataset que contiene información detallada de estudiantes de bachillerato, incluyendo características demográficas, hábitos de estudio, nivel de participación parental, actividades extracurriculares y rendimiento académico final. Fue diseñado específicamente para tareas de clasificación y predicción de desempeño estudiantil.
...
- **Variables de entrada principales (X):**
  - `StudyTimeWeekly`: Horas de estudio semanales del estudiante (numérica continua)
  - `Absences`: Número de ausencias registradas durante el año escolar (numérica discreta)
  - `Tutoring`: Si el estudiante recibe tutorías adicionales (binaria: 0=No, 1=Sí)
  - `ParentalSupport`: Nivel de apoyo y participación de los padres (ordinal: 0=None, 1=Low, 2=Moderate, 3=High, 4=Very High)
  ...
  - `Volunteering`: Participación en actividades de voluntariado (binaria: 0=No, 1=Sí)
  - `Age`: Edad del estudiante (numérica discreta)
  - `Gender`: Género del estudiante (binaria: 0=Male, 1=Female)
  - `Ethnicity`: Grupo étnico del estudiante (categórica: 0–3)
  - `ParentalEducation`: Nivel educativo de los padres (ordinal: 0=None, 1=High School, 2=Some College, 3=Bachelor's, 4=Higher)
...
**Riesgo identificado:** El modelo puede generar predicciones sesgadas hacia grupos étnicos o socioeconómicos específicos si las variables `Ethnicity` y `ParentalEducation` tienen alta correlación con la variable objetivo. Esto podría llevar a estigmatizar a ciertos grupos de estudiantes.

**Acción de mitigación:** Se realizará un análisis de equidad (fairness analysis) por grupos étnicos y niveles de educación parental. Si se detecta sesgo estadísticamente significativo, las variables `Ethnicity` y `ParentalEducation` serán excluidas del modelo final. Adicionalmente, los resultados del clasificador se presentarán como una herramienta de apoyo a la decisión docente, no como un veredicto definitivo sobre el estudiante.

---

## 10. URL del repositorio GitHub

https://github.com/MAIGUEL2521/-sistema-riesgo-acad-mico
